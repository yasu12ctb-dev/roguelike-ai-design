...
- **ボス**：**エリアボス**（深度8毎・下り階段を守る山場＝`Ω`紫紅。過去の探索者化石の名を冠す）／**中ボス**（奥の増強個体＝金）。撃破は大XP＋年代記に刻まれる（4-11F）
- **装備**：武器（攻+）／防具（被ダメ-）／遺物（パッシブ）。宝箱＋ボスドロップ。**異物＝未鑑定**（装備で判明・副作用も）。**死亡時、握っていた武器が化石に刻まれる**＝継承で実際に奪還・再装備（4-11E）
- **鎮め筋**：敵性化した探索者ボスは、削り切った瞬間に**「討つ／鎮める」**を選べる（戦闘版の干渉動詞）。討つ＝XP満額＋遺物、鎮める＝非殺・年代記に残る（4-11D）

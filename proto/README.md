...
## Web版（本体）
- **NetHack型のグリッドダンジョン**：小部屋＋1マス幅通路＋行き止まり＋ループの手続き生成（最近傍接続で必ず連結）、`@`のターン制移動（スワイプ／タップで一歩・自分タップで待機／矢印キー）、視界(FOV)と記憶表示、階段で深度が実体化。**マップは深いほど広く、`@`を追うカメラがスクロール**（「図」＝実マップを 1:1 でそのまま俯瞰）。敵数はマップ面積＋深度でスケール

...
## Web版（本体）
- **NetHack型のグリッドダンジョン**：部屋＋通路の手続き生成、`@`のターン制移動（スワイプ／タップで一歩・自分タップで待機／矢印キー）、視界(FOV)と記憶表示、階段で深度が実体化。**マップは深いほど広く、`@`を追うカメラがスクロール**（「図」で全体を俯瞰）
- **化石はマップ上の実体（†）**：歩み寄ると再発見シーン（鎮魂/継承/立ち去る）が開く
- モンスター数体とテレグラフ戦闘（敵の次手が見える・確定命中/確定ダメージ・退けば見切り：4-11A）、深蝕と奇癖、死→最後の一手→世代交代（系譜）→年代記
- 見た目＝方向A（発光グリフ）・縦持ち。「図」＝踏破範囲の俯瞰地図、メニュー（≡）＝年代記＋今後の拡張フック

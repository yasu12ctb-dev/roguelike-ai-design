...
export interface Floor {
  ...
  echo?: { // 最期の残響（RFC・P1）：配置済み自血統化石の1体に生える残響（web が enterFloor 初訪で computeEcho から写す＝engine 非使用・golden 安全＝aurelSite と同じパターン）
    fossilId: string; x: number; y: number; kind: "calm" | "will" | "guard" | "curse";
    loot?: { x: number; y: number; gear: string; taken?: boolean };  // 守り手：遺品マス（先代の gearTags を itemByName で復元）
...
/** 部屋内のランダムな空き床（避けたい点から minDist 以上・敵の居ないマス）。見つからなければ null（呼び側で全域散布へフォールバック）。 */
function randomTileInRoom(f: Floor, rng: Rng, r: Room, avoid: Pos, minDist: number): Pos | null {
  for (let t = 0; t < 12; t++) {
    const x = r.x + rng.int(r.w), y = r.y + rng.int(r.h);
...
export function genFloor(world: World, depth: number, opts?: { abyss?: boolean; fodderMul?: number; shapedWeight?: number; burstWeight?: number; roomBias?: number; routeGuards?: number }): Floor {
  ...
  const floor: Floor = {
    ...
    rooms: rooms.map((r) => ({ x: r.x, y: r.y, w: r.w, h: r.h })), // 部屋aggro が読む（golden の genFloor ハッシュは rooms を読まない＝不変）
  };
  ...
  if (depth >= FODDER_MIN_DEPTH && fodderMul > 0) {
    ...
    if (fodderPool.length) {
      ...
      let cap = monsterHardcap(depth, mods) - floor.monsters.length; // C｜深部で天井を上げる（easy=60 据え置き＝golden 不変）
...
export function planMonsters(f: Floor, player: Pos, rng: Rng, companion?: CompanionEntity | null, allies?: readonly CompanionEntity[] | null): void {
  ...
  // ★部屋aggro（v0.151.0・2026-07-11 ユーザー承認＝トルネコ的「部屋＝戦場／通路＝安全地帯」）：プレイヤーが部屋に入ったら
  //   その部屋内の眠っている敵を一斉に起こす（個別のLOS/FOVを待たず、部屋全体が同時に迫る＝広間の緊張）。通路は従来どおり
  //   個別視認で1体ずつ＝引き込んで1対1にする立ち回りは温存。★normal/hard 限定（f.diff!==EASY_MODS）＝easy/golden は不変。
  if (f.diff && f.diff !== EASY_MODS && f.rooms) {
    const pr = f.rooms.find((r) => player.x >= r.x && player.x < r.x + r.w && player.y >= r.y && player.y < r.y + r.h);
    if (pr) for (const m of f.monsters) {
      if (!m.awake && m.hp > 0 && m.x >= pr.x && m.x < pr.x + pr.w && m.y >= pr.y && m.y < pr.y + pr.h) m.awake = true;
    }
  }
  for (const m of f.monsters) {
    ...
    if (m.boss === "area") { // ボスの戦術化（B）：覚醒判定・大技CD・眷属召喚。全て area ボス限定＝通常敵/golden 不変。
      ...
      if (m.enraged) { // ③眷属召喚（覚醒後のみ・breederMinion 流用・総数上限）
        ...
        else if (rng.next() < BOSS_SUMMON_CHANCE && f.monsters.length + births.length < monsterHardcap(f.depth, f.diff ?? EASY_MODS)) {
          let placed = false;
    ...
    if (m.kind.ability === "breeder") { // 孵化（4-11G）：たまに隣の空き床へ弱い眷属（総数上限＋クールダウン）。湧いても自分は普通に行動する。
      ...
      else if (rng.next() < BREED_CHANCE && f.monsters.length + births.length < monsterHardcap(f.depth, f.diff ?? EASY_MODS)) {
        let placed = false;
        for (let dy = -1; dy <= 1 && !placed; dy++) for (let dx = -1; dx <= 1 && !placed; dx++) {
          if (dx === 0 && dy === 0) continue;
          const cx = m.x + dx, cy = m.y + dy;

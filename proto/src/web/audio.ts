...
let master: GainNode | null = null;     // ミュート反映（全体）
let sfxBus: GainNode | null = null;     // 効果音専用ボリューム（master の下＝BGM と独立に調整）
let sfxVol = 0.85;
let muted = false;
let started = false;
...
export function ensureAudio(): void {
  ...
  try {
    ...
    sfxBus = ctx.createGain();           // 効果音はこのバス経由＝BGM(bgmGain) と独立に音量を持つ
...
function getSfxRev(): ConvolverNode | null { // 「節目の音」用の短い余韻（lazy）
  ...
  if (!sfxRev) {
    ...
    const ret = ctx.createGain(); ret.gain.value = 0.5; sfxRev.connect(ret); ret.connect(sfxBus!);
  }
  return sfxRev;
}
...
function thud(freq: number, dur: number, peak: number, rev = 0): void {
  ...
  const g = ctx.createGain(); env(g, peak, dur);
  o.connect(g); g.connect(sfxBus!); if (rev) revSend(g, rev);
  o.start(); o.stop(ctx.currentTime + dur + 0.03);
}
// 余韻つきの澄んだ単音（鐘・儀式・節目）。
...
const SFXVOL_KEY = "sekitsui.sfxvol";
let bgmEnabled = true;
let bgmVol = 0.6;
...
export function sfxVolume(): number { return sfxVol; }

/** 効果音の音量（BGM と独立）。0..1。 */
export function setSfxVolume(v: number): void {
  sfxVol = Math.max(0, Math.min(1, v));
  try { localStorage.setItem(SFXVOL_KEY, String(sfxVol)); } catch { /* ignore */ }
  if (sfxBus && ctx) sfxBus.gain.setTargetAtTime(sfxVol, ctx.currentTime, 0.05);
}
...
export function loadBgmPref(): void {
  try {
    ...
    const s = localStorage.getItem(SFXVOL_KEY); if (s !== null) { const n = parseFloat(s); if (!Number.isNaN(n)) sfxVol = Math.max(0, Math.min(1, n)); }
  } catch { /* ignore */ }
}

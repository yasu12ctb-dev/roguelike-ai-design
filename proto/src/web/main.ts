...
async function characterCreation() {
  ...
  // 序盤の丸腰を解消（テストプレイFB 2026-07-03・案A→中間調整）：支給は**武器のみ（短刀＝攻+1・鑑定済み）**。
  // 防具は「見つける」対象に戻す（初期支給+宝箱率アップの二重で緩過ぎたFBを受け、中間値へ引き戻し）。
  // 継承で得た装備があればそちらを優先＝上書きしない。支給は最弱基ゆえ「終始シビア」は崩さない（序盤だけ緩める）。
  ...
  const intro = lineage.relation === "none"
    ? `${ch.name}は、何も受け継がず、支給の短刀ひとつを手に迷宮へ向かう（${statsLine(ch)}）。`
...
async function chestScene(ce: Chest) {
  ...
  if (r.pick === 1) {
    ...
    } else if (roll < 0.58) { // 装備ドロップ（序盤の入手を底上げ・約28%→約31%＝初期値0.55と0.62の中間・FB 2026-07-03 案B→中間調整）

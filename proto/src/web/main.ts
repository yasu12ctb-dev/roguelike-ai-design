...
function clearRaidBoard(): void {
  ...
  armorBuffTurns = attackBuffTurns = hasteTurns = deathDoorTurns = poisonTurns = poisonDmg = shadowGuard = counterTurns = parryTurns = 0; guardArmed = false;
  summons = [];
}
...
async function raidDefeat(): Promise<void> {
  ...
  armorBuffTurns = attackBuffTurns = hasteTurns = deathDoorTurns = poisonTurns = poisonDmg = shadowGuard = counterTurns = parryTurns = 0; guardArmed = false; summons = [];
  ...
  if (crit) {
    ...
    for (const m of [center, ...shoulders]) {
      if (m && m.hp > 0) {
        ...
        m.stunned = Math.max(m.stunned ?? 0, NAGINATA_STAGGER); staggered++;
      }
    }
...
async function endTurn() {
  ...
  if ((world.difficulty ?? "easy") !== "easy") {
    wanderTurns++;
    if (wanderTurns % WANDER_EVERY === 0 && wanderCount < WANDER_FLOOR_CAP && floor.monsters.filter((m) => m.hp > 0).length < MONSTER_HARDCAP) {
  ...
  for (const h of res.hits) {
    if (h.target === "companion" && companion) {
      ...
      if (guarded) dmg = Math.ceil(dmg / 2); // 剣＝受け流し（v0.149.0＝半減・下限1は ceil が担保）

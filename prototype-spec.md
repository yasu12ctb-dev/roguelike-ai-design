...
**★本表は抜粋ではなく網羅的な正典（`@keyframes` 全 29 件）＝実装を正典にしない**（U1c の教訓＝宣言を正典に置き、実装との差は機械検査で落とす）。**`@keyframes` を足す／消す／分類を変えるときは本表を先に更新する。** `tools/a11y-check.ts`（`npm run check` 同梱）が `web/index.html` の `@keyframes` 集合と本表の ID 集合を突合し、**片側にしか無い ID があれば fail**（＝表に無いアニメは追加できない）。

**Reduce Motion 分類（列 `RM`）＝ OS の Reduce Motion／`prefers-reduced-motion: reduce` 時の扱い。**
- **A＝装飾**：情報を担わない雰囲気。**停止**（静的な最終状態で置く）。
- **B＝状態・予告**：情報を担う。**動きは止めるが、静的な代替表現（枠・塗り・記号）を必ず用意する**＝「来る」が読めなくなってはならない。**予告が読めない＝難易度低下ではなく理不尽化**（守るゲーム原則「予告・地形・位置取りで戦う」）。
- **C＝単発フィードバック**：短命の結果通知。**移動・拡縮・回転・振動は停止**（前庭刺激）。**不透明度のフェードと表示自体は残してよい**（情報の消失を防ぐ）。

| keyframe | 周期（実値） | RM | 意味・対象 |
|---|---|---|---|
| `pulse` | 1.05–2.8s（対象ごと） | A | 重要物の「呼吸」＝化石・泉・安息所・扉・召喚・敵 t5（2s）・エリート（1.4s）・エリアボス（1.05s）・erratic 相棒（1.1s）等。**停止しても記号字形＋ティア色の二重符号化（10.11）で識別は落ちない** |
| `abyssair` | 6s | A | 深淵帯の菫ヴィネット脈動（10.3b） |
| `torchflick` | 4.2s | A | 松明のゆらぎ（10.3b） |
| `echoaura` | 3.4s | A | 最期の残響のトーン色オーラ（4-11A） |
| `hzfire` / `hzvenom` / `hzmiasma` / `hzfrost` | 1.3s / 2.2s / 2.6s / 2.4s | A | 地形ハザード（業火床・毒沼・蝕の霧・凍霧）。**危険の所在は静的な背景色が担う**ため停止可 |
| `danger` | .55s（ボス大技の被予告は .4s） | **B** | 被攻撃予告の自分/相棒が赤く脈打つ |
| `monatk` | .5s | **B** | 敵の攻撃予告（ティア色のまま強く明滅＝「来る」） |
| `bossheavy` | .42s | **B** | ボスの渾身の一撃（溜め大技）の予告 |
| `tele` | .55s | **B** | 討たれるマスの赤枠（`.cell.tele-atk::after`） |
| `teleboss` / `teleshape` / `telecharge` / `telereach` | .5s / .5s / .45s / .5s | **B** | 形つき確定範囲（ボス）／形つき敵（arc・slam・beam）／突進ライン／長柄の突き線（4-11A） |
| `throwaim` | .7s | **B** | 投擲の照準マス（到達可否の判断材料） |
| `hzcrack` | .5s | **B** | 崩れ床が軋む＝**次に落ちる**警告（他ハザードと違い状態遷移の予告） |
| `lungeGlow` | 1.1s | **B** | 踏み込みの**構え中**というモード表示（`#lungeBtn.stance`） |
| `floatup` | .55–.8s | C | FloatFx の浮上フェード（与ダメ/被ダメ/回復/見切り/撃破） |
| `critpop` | .28s | C | 会心数字の字送り＋輝度 |
| `shakecrit` | .14s | C | 会心時の盤面マイクロシェイク（±2px）＝**RM 時は完全停止**（前庭刺激が最も強い） |
| `fxready` / `fxslash` / `fxpressL` / `fxpressR` | .45s / .3s / .26s / .26s | C | 位置取り演出（見切り波紋・会心斬光・挟撃の楔・10.3） |
| `fxflash` | .4–.55s | C | 術の全域点滅（warp 菫／still 青／blink 青菫） |
| `peekin` | .16s | C | 「調べる」ポップの出現 |
| `bannerfade` | 1.5s | C | フロア進入バナー（10.3b） |
...
Swift ＝ `withAnimation(.easeInOut.repeatForever())` 等でミラー。**Reduce Motion（`.accessibilityReduceMotion`）時は上の RM 列に従う**＝A/C は停止、**B は静的表現へ置換**（10.11）。
...
- **測定条件**＝盤面トークンは `bg-void` と `bg-wall` の**両方**で判定（最悪ケース＝`bg-wall`）。シート系は `bg-sheet`、HUD 系は `bg-panel`。
- **正典＝本表**。`tools/a11y-check.ts` が既定値・高コントラスト値の双方を実 CSS から読み、**WCAG 比を毎回再計算して基準未達なら fail**（＝色を触った瞬間に落ちる）。**表と CSS の値が食い違っても fail。**
...
**★実装状況（U1b・2026-07-28）：高コントラストと Reduce Motion は web 側で実装済み＝Swift が鏡にする「実行可能な仕様」。** 値と分類の正典は **10.2f（高コントラスト・トークン差分 6 件）** と **10.2d（`RM` 列・`@keyframes` 全 29 件）**、機械検査は `tools/a11y-check.ts`（`npm run check` 同梱）。**Dynamic Type と VoiceOver は Swift フェーズ（U2）**＝web の DOM では等価物を持てないため。
...
- **Reduce Motion**（`prefers-reduced-motion: reduce` / `.accessibilityReduceMotion`）**＝分類は 10.2d の `RM` 列が正典**（A=装飾は停止／**B=状態・予告は静的表現へ置換**／C=単発は移動・拡縮・回転・振動を停止しフェードは許容）。**テレグラフ（B）を「消す」実装は禁止**＝「来る」が読めなくなるのは難易度低下ではなく理不尽化。

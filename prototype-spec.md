...
- **余白の三注記＝初回体験のオンボーディング**（`showGuideOnce(flag,text)`・v0.153.0・PR4）：モーダルを増やさず、状況が起きた時に一度だけ**ログ cue** で最小語彙を教える（Apple のゲーム向けオンボーディング指針＝コアループを一段ずつ・必要時に文脈補助）。**入力・手番・RNG・盤面を一切消費しない**（`log` と `world.flags` のみ）。`guide_active`（`maybeIntro` 末で新規ワールドのみ立つ）が無ければ何も出さない＝既存セーブ/veteran には出ない。4トリガー＝A `guide_town_seen`（初回の街＝記号 @ と操作）／B `guide_intent_seen`（FOV内に初の攻撃予告＝退けば見切り・踏み込めば攻撃）／C-1 `guide_dodge_seen`（初の見切り成功＝反撃の好機）／C-2 `guide_kill_seen`（初の非ボス撃破＝地図で > を探す）。各注記は個別 flag で一度きり（リロード/次世代で再表示せず・ワールドリセットで再表示）。Swift も同じ4トリガー＋一度きり flag をミラー。
...
- **共通UI**：選択肢シート `sheet({text, sections?, meta, options})`（全対話の基本単位）。**三層構造（横断F ①・v0.96.0）＝①ヘッダ帯（`#sheetHead`＝題〔朱の左罫〕＋副題＋✕。`meta` を最初の `" ── "` で題／副題に分割）／②本文 `#sheetText`（自由文）＋任意の構造化リスト `#sheetList`（kv 行・セクション見出し）／③役割つきボタン列 `#sheetButtons`。** Swift も「ナビゲーション題（`.topBarTrailing` dismiss）＋本文/セクション＋ボタン列」を基本コンポーネントにする。
  - **詰め版（`sheet({compact:true})`・横断F ②・v0.171.0／満載状態への対応は v0.172.0）：** 情報量が多く一画面に収まらない画面だけの変種（CSS `#sheet.compact`）。**基準＝375×812（実機相当の最小）で一画面に収まること。★「最も節と選択肢が増える状態」で満たすこと**＝ステータスなら相棒を雇用中かつ記憶ありの世界（`tools/e2e-ui-prefs.ts` が〈標準／満載〉×〈375×812／480×900〉の4通りで実測固定。**最短ケースだけで測って「収まった」ことにしない**）。内容＝①kv 行と選択肢の余白を詰める ②**短い選択肢を2列**に並べる ③補助の淡色行（`kvtext.dim`）は**出さない**（全文は詳細画面で読める）。**`min-height 48px`（タッチ最小）は不変。** 全幅になるのは **`cancel`／設定見出し／`wide` を付けた選択肢**の3種。**物語の選択肢（長文）には掛けない**＝現在の採用は**ステータス（`charScreen`）のみ**。`chooseGrid`（カード一覧）は必ず詰め版を外す。ステータス側は行そのものも集約する（HP/攻撃→能力行／荷物＝枠数と中身を自分の節の1行／**相棒＝名＋絆の1行**〔等級・偉業は「相棒を見る」で読む〕／**記憶は行も節も作らない**〔件数は「記憶を見る（N）」ボタンが出す＝同じ数字を二度出さない〕）＝**節の見出しは「装備」「術」の2つだけ**＋**装備の値は銘・+N までの短名（`itemShort`）**＝性能説明（攻＋5・薙ぎ払い…）は詳細（`gearSheet`）で読む（性能つき `itemLabel` を要約に出すと装備行が数行に伸びる＝375px 実測 +86px。未鑑定の伏せ方は `itemLabel` と同じ）。**Swift ＝ 行 inset を詰めた Form ＋ 2 列 LazyVGrid のボタン列。**
    - **`wide`（選択肢）＝2列組みで1つだけ全幅にする。** 長いラベルは半幅に収まらず折り返して**行の高さを押し上げる**（2列 grid は行内で高さが揃うため、隣の短いラベルまで巻き込む）。375px 実測で「進行中（依頼・因縁・印）」が 66px → `wide` で 48px。通常シート（1列）では no-op。
    - **`cols2`（節）＝その節の kv 行を2列に並べる（詰め版のみ有効）。** 同型の枠が並ぶ節にだけ使う（現＝装備の武器/防具/遺物/鞄）。**本文の数値行には使わない**（値の位置が左右にぶれて視線が散るため）。2列の先頭行は左右とも上罫を出さない（`.first` は先頭1つにしか付かないので左右非対称になる）。**Swift ＝ `LazyVGrid(columns: 2)` の Section。**
...
| `abyssair` | 6s | 深淵帯の菫ヴィネット脈動（10.3b） |
| `torchflick` | 4.2s | 松明のゆらぎ（10.3b） |
| `echoaura` | 3.4s | 最期の残響のトーン色オーラ（4-11A） |
| `hzfire` / `hzvenom` / `hzmiasma` / `hzfrost` | 1.3s / 2.2s / 2.6s / 2.4s | 地形ハザード（業火床・毒沼・蝕の霧・凍霧） |
...
| `throwaim` | .7s | 投擲の照準マス（到達可否の判断材料） |
| `hzcrack` | .5s | 崩れ床が軋む＝**次に落ちる**警告（他ハザードと違い状態遷移の予告） |
| `lungeGlow` | 1.1s | 踏み込みの**構え中**というモード表示（`#lungeBtn.stance`） |
| `floatup` | .55–.8s | FloatFx の浮上フェード（与ダメ/被ダメ/回復/見切り/撃破） |
| `critpop` | .28s | 会心数字の字送り＋輝度 |
| `shakecrit` | .14s | 会心時の盤面マイクロシェイク（±2px） |
| `fxready` / `fxslash` / `fxpressL` / `fxpressR` | .45s / .3s / .26s / .26s | 位置取り演出（見切り波紋・会心斬光・挟撃の楔・10.3） |
| `fxflash` | .4–.55s | 術の全域点滅（warp 菫／still 青／blink 青菫） |
| `peekin` | .16s | 「調べる」ポップの出現 |
| `bannerfade` | 1.5s | フロア進入バナー（10.3b） |

**★Reduce Motion 分類は keyframe ではなく「適用セレクタ」に与える。** 同じ keyframe を共有していても**用途が違えば分類は異なる**（`pulse` が典型＝場所ノードの装飾と、化石の未鎮め表示の両方に使われている）。分類の判定規則は次の一文：

> **既定モードで区別できていた組が、アニメ停止によって区別できなくなるなら B。**（＝**アニメの有無が識別に寄与しているか**。単に「重要な物」かどうかではない）

- **A＝装飾**：情報を担わない雰囲気、またはグリフ字形が一意で停止しても識別が落ちないもの。**停止**。
- **B＝状態・予告**：情報を担う。**動きは止めるが、静的な代替表現（枠・塗り・発光の固定）を必ず用意する**＝「来る」「これは何か」が読めなくなってはならない。**予告が読めない＝難易度低下ではなく理不尽化**（守るゲーム原則「予告・地形・位置取りで戦う」）。
...
| RM | セレクタ（＝分類の単位） | 根拠 |
|---|---|---|
| **A** | `.g-downed`／`.g-summon`／`.g-delver`／`.g-spring`／`.g-rest`／`.g-door`／`.g-aurel`／`.g-laila` | 字形が一意（`&`・`ψ/‡/Ψ`・`泉`・`安`・`扉`・`痕`・`境`）または色が静的に一意（`.g-delver` の緑）。停止しても識別は落ちない |
| **A** | `#mapWrap.band-abyss::after`／`#light`／`.cell.echo-loss::after`／`.cell.echo-myth::after`／`.cell.echo-grudge::after`／`.cell.hz-fire`／`.cell.hz-venom`／`.cell.hz-miasma`／`.cell.hz-frost` | 雰囲気。ハザードの**所在は静的な背景色**が担う |
| **B** | `.g-fossil` | `†` を `.g-fossil-quiet`（静的）と共有＝**アニメの有無が「まだ鎮めていない」の識別そのもの**（`quiet` の命名が示すとおり） |
| **B** | `.g-companion-erratic` | `@` を player／companion と共有。奇癖＝「何かおかしい」の合図が脈動 |
| **B** | `.g-mon-t5`／`.g-elite`／`.g-boss` | 同種の格上は**記号が同一**で、既定では脈動も差になっている |
...
### 10.2f 高コントラスト（`prefers-contrast: more` / iOS *Increase Contrast*）
**方針＝配色を総取り替えしない。** 実測（WCAG 2.x 相対輝度・対象別基準は 10.11）で、**前景トークンの大半（52 件中 45 件）は既定のまま基準を満たす**。**満たさない 7 件だけを、色相・彩度を保ったまま明度で持ち上げる**＝役割の対応（自分=金系・深蝕=菫系・敵ティアの段・術学派の別）と「静謐な写本」の空気を壊さない。

| トークン | 既定 | 実測 | 高コントラスト値 | 実測 | 必要 | 想定背景 |
|---|---|---|---|---|---|---|
| `--tx-meta` | `#857a66` | 4.38:1 | **`#988d79`** | 5.67:1 | 4.5:1（文字） | `bg-sheet` |
| `--tx-faint` | `#6b6250` | 3.07:1 | **`#897d66`** | 4.57:1 | 4.5:1（文字） | `bg-sheet` |
| `--line` | `#372f23` | 1.40:1 | **`#716048`** | 3.06:1 | 3:1（非テキスト） | `bg-sheet`/`bg-panel` |
| `--line-2` | `#4a3f2e` | 1.80:1 | **`#8b7656`** | 4.26:1 | 3:1（非テキスト） | 同上 |
| `--line-3` | `#4a3f2e` | 1.80:1 | **`#8b7656`** | 4.26:1 | 3:1（非テキスト） | 同上（`--line-2` と同値の別名） |
| `--g-wall` | `#39434f` | 1.82:1 | **`#708298`** | 4.66:1 | 3:1（非テキスト） | `bg-void`/`bg-wall` |
| `--g-floor` | `#2c333d` | 1.44:1 | **`#576578`** | 3.07:1 | 3:1（非テキスト） | 同上 |

- **段差（階層）を潰さない**＝同じ群のトークンは基準ぎりぎりで揃えず、**既定と同じ明暗の順序を保つ**（`tx-meta` > `tx-faint`／`line-2` > `line`／`g-wall` > `g-floor`）。壁と床が同じ明るさになると盤面の構造が読めなくなり、10.11「色を唯一の識別手段にしない」に反する。
- **既定テーマの値は変えない**（雰囲気優先＝OS 設定 ON のときだけ差し替わる）。**設定シートに手動トグルは置かない**（OS 設定に一本化＝10.7 の 17 項目・保存キー・U1a の凍結 oracle・U1c fixture はいずれも無改変）。

**★閾値と判定面の規則（閉じた表＝全前景トークンがちょうど1行に当たる）。** 閾値は「その群の代表用途」ではなく **実際の使用箇所のうち最も厳しい役割**を採る。判定面も**実際に載る面のうち最も明るいもの**を採る。**`:root` の色変数がどの行にも当たらない／複数行に当たる場合は検査 fail**（＝トークンを足して閾値の割り当てを忘れられない）。

| 群 | 判定面（実使用のうち最も明るい面） | 閾値 | 最も厳しい用途の実例 |
|---|---|---|---|
| `bg-*` | — | **対象外** | 面そのもの |
| `tx` / `tx-strong` / `tx-2` / `tx-dim` / `tx-status` / `tx-log` / `tx-log-dim` / `tx-meta` | `bg-sheet` `#17130e` | 4.5:1（文字） | シート本文・ログ・`#sheetHeadSub` 10.5px |
| `tx-faint` | **`bg-wall` `#11151c`** | 4.5:1（文字） | **`.fl-miss`（盤面に出る 12px の「見切」）**＝盤面テキストゆえ面が `bg-wall` |
| `line` / `line-2` / `line-3` | `bg-sheet` `#17130e` | 3:1（非テキスト） | 罫線・ゲージ枠のみ（文字色に使わない） |
| `acc` | `bg-sheet` `#17130e` | 3:1（非テキスト） | 見出しの左罫・primary ボタンの枠のみ（文字は literal 色） |
...
|---|---|---|---|---|
| `#aimBar #aimGo` | `color` | `on:#aimBar` `over:bg-void` | 4.5:1（文字） | 射線バーの「撃つ」 |
| `#throwBar #throwGo` | `color` | `on:#throwBar` `over:bg-void` | 4.5:1（文字） | 投擲バーの「投げる」 |
| `.g-aim` | `color` | `js:MAP_BG.aimOk` `js:MAP_BG.aimNg` `bg-void` | 4.5:1（文字） | 地図の照準記号と凡例 |
| `#floorBanner.abyss` | `color` | `bg-void` `bg-wall` | 4.5:1（文字） | 深淵帯の帯文言 |
| `#floats .fl-kill` | `color` | `bg-void` `bg-wall` | 4.5:1（文字） | 盤面の撃破表示 |
| `#floats .fl-crit` | `color` | `bg-void` `bg-wall` | 4.5:1（文字） | 盤面の会心数字 21px |
| `@keyframes danger 50%` | `color` | `bg-void` `bg-wall` | 4.5:1（文字） | 危機の点滅（@・相棒・大技） |
| `.g-boss-heavy` | `color` | `bg-void` `bg-wall` | 3:1（非テキスト） | 盤面グリフ（根拠は 10.2f 末尾と同じ） |
| `.g-player-heavy` | `color` | `bg-void` `bg-wall` | 3:1（非テキスト） | 同上 |
| `.cell.throw-aim::after` | `color` | `on:.cell.throw-aim::after` `over:bg-wall` | 4.5:1（文字） | 投擲候補マスの記号 |
| `#tabbar #spellBtn` | `color` | `bg-panel` | 4.5:1（文字） | タブのラベル |
| `#tabbar #mapBtn` | `color` | `bg-panel` | 4.5:1（文字） | タブのラベル |
| `#lungeBtn` | `color` | `on:#lungeBtn` `over:bg-app` | 4.5:1（文字） | 踏み込みボタン |
| `#lungeBtn.stance` | `color` | `on:#lungeBtn.stance` | 4.5:1（文字） | 構え中の踏み込みボタン |
| `#guardBtn` | `color` | `on:#guardBtn` `over:bg-app` | 4.5:1（文字） | 受け流しボタン |
| `#dpad button` | `color` | `on:#dpad button` `over:bg-app` | 4.5:1（文字） | 方向キー |
| `#dpad button:active` | `color` | `on:#dpad button:active` `over:bg-app` | 4.5:1（文字） | 方向キー押下 |
| `#dpad button.center` | `color` | `on:#dpad button` `over:bg-app` `on:#dpad button:active` `over:bg-app` | 4.5:1（文字） | 中央キー（押下面にも載る） |
| `.selgrid .cost` | `color` | `bg-sheet` `bg-btn-active` | 4.5:1（文字） | カードの消費表示 |
| `.selgrid .mc .icn` | `color` | `bg-sheet` `bg-btn-active` | 4.5:1（文字） | カードのシジル |
| `#sheetButtons button.b-primary` | `color` | `bg-sheet` `bg-btn-active` | 4.5:1（文字） | primary ボタンの文字 |
| `#title .embers span` | `color` | — | **免除** | `display:none` の死蔵（別テーマ復帰用に DOM/CSS を残している・10.2b） |
...
**正典＝本節の表。** `tools/a11y-check.ts` が①実 CSS から母集合を抽出して表と 1:1 突合（欠け・余り・重複は fail）②判定面を上の 4 形態の文法だけ受け付けて解決（当たらない書き方は fail）③実ソースの色で WCAG 比を毎回再計算（未達は fail）を行う。**新しいハードコード前景色を書きたくなったら、先に本表へ行を足す。**
...
- **地図はパン可能（v0.109.0）**：フロアを**読めるサイズ（1タイル ≥ `MAP_CELL_MIN`=13px）で固定**し、地図エリアに収まりきらなければ **@ 中心のカメラ窓**（`mapCam`/`mapCols`×`mapRows`）で切り出す＝**ドラッグでスクロール**（`layoutMapView`/`centerMapOn`/`ensureMapVisible`・タッチ `touchmove` で `mapCam` を指と逆に移動）。収まる浅いフロアは全体表示（グリッド＝フロア寸法）。深層でフロアが広がっても潰れず読める（旧＝フロア全体を固定枠に縮小＝深層で1タイル数pxに潰れる問題への対応）。パン可能時は凡例に「◧ ドラッグでスクロール」（`#mapPanHint`）。**全体図トグル（`#mapOverviewBtn`・右下・v0.110.0）**＝「全体図」でフロア全体を1画面に収める（`mapOverview`＝`cell=fitCell`・パン無し）／「拡大」で読めるサイズ+パンへ戻る（@ 中心へ再センタリング）。地図を開くたび既定は拡大（パン）表示。
...
- **Reduce Motion**（`prefers-reduced-motion: reduce` / `.accessibilityReduceMotion`）**＝分類は 10.2d の「セレクタ単位の RM 分類表」が正典**（A=装飾は停止／**B=状態・予告は静的表現へ置換**／C=単発は移動・拡縮・回転・振動を停止し表示は残す／不透明度のみのフェードは免除）。**分類は keyframe でなくセレクタに与える**（同じ keyframe でも用途が違えば分類が異なる）。**テレグラフ（B）を「消す」実装は禁止**＝「来る」が読めなくなるのは難易度低下ではなく理不尽化。

...
- **共通UI**：選択肢シート `sheet({text, meta, options})`（全対話の基本単位＝本文＋見出し＋ボタン列）。Swift も「テキスト＋選択肢ボタン列」を基本コンポーネントにする。
  - **上部固定の「閉じる」ショートカット（v0.76.0・2026-06-29）：** シートは下端アンカーで、選択肢が多いと「閉じる」が最下部に来て押しづらい。そこで**シート最上部・右肩に sticky の専用「✕ 閉じる」ボタン**を出す（スクロールしても常に手の届く位置）。出す条件＝**選択肢が1つだけ（＝それが唯一の出口）か、末尾が閉じ系の語（閉じ/戻る/やめ/立ち去/出る/済ま/後で/いいえ/よす/帰る）のとき**だけ＝強制選択（討つ/鎮める等）では出さない。動作＝対応する閉じ系の選択肢（or `chooseGrid` のキャンセル＝-1）と同一＝意味は不変、上部からも押せるだけ。**Swift では sheet のツールバー dismiss（`.topBarTrailing`）にミラー**＝移植で無駄にならない。最下部の閉じる選択肢は据え置き（冗長だが非破壊）。

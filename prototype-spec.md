...
**方針＝配色を総取り替えしない。** 実測（WCAG 2.x 相対輝度・対象別基準は 10.11）で、**前景 51 トークン中 45 は既定のまま基準を満たす**。**満たさない 6 件だけを、色相・彩度を保ったまま明度で持ち上げる**＝役割の対応（自分=金系・深蝕=菫系・敵ティアの段・術学派の別）と「静謐な写本」の空気を壊さない。

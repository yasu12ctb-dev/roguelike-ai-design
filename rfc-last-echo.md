...
**status: 方向性承認済み（2026-07-18 ユーザー承認＝推奨案どおり・実装は別途着手判断）／対象 v0.155.0（HEAD 20b0b6c）**
承認された推奨＝P1 は4種全部／遺言の一行入力あり（スキップ可・空なら定型文）／対象は自血統限定／数値は全てテスト調整候補。
`design-snapshot.md`（正典）への反映は実装 PR と同時。

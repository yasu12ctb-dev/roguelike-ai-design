...
**status: v3 承認済み（Codex 2026-07-27 再検収 OK）・PR #392 merge 済み。Phase U0（doc 整備）実施中→本 RFC 内容は `prototype-spec.md` §10 へ転記済み・`design-spec.md` はアーカイブ化済み。U1a/U1b/U1c・U2 は各個別承認（未着手）。** 起点＝Obsidian 台帳 2026-07-19 18:50 [codex] のUI全体監査。方向性承認＝2026-07-26。v2＝00:51 [codex]／v3＝01:34 [codex] の修正必須を反映。**このRFCは実装しない**（実装は U1/U2 の個別承認で）。

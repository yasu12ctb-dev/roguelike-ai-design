...
**status: v3 承認済み（Codex 2026-07-27 再検収 OK・PR #392）。Phase U0（doc 整備＝本 RFC 内容を `prototype-spec.md` §10 へ転記＋`design-spec.md` のアーカイブ化）はユーザー承認済みで、PR #393 にて正典反映を提案（merge 状況は当該 PR を参照）。U1a/U1b/U1c・U2 は各個別承認（未着手）。** 起点＝Obsidian 台帳 2026-07-19 18:50 [codex] のUI全体監査。方向性承認＝2026-07-26。v2＝00:51 [codex]／v3＝01:34 [codex] の修正必須を反映。**このRFCは実装しない**（実装は U1/U2 の個別承認で）。**転記後の UI 正典は §10＝以後の更新は §10 側で行い、本 RFC は経緯・根拠の記録とする。**
...
2. **§5 A11y（v3 修正4）**：D-pad「8方向＋待機」表記／VoiceOver 通知の coalesce・重複抑制・優先通知（被弾/拒否/深度移動）／高コントラストの対象別基準（通常文字 4.5:1／大きな文字〔18pt 以上・14pt 以上の太字〕3:1／非テキスト UI 3:1）＋色を唯一の識別手段にしない（記号/ラベル/形の併用）が妥当か。

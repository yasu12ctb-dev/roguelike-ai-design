...
1. **店・施設の他動詞の肉付け（要・小設計）**：慰霊堂（鎮魂・供養）／教団（深蝕と引き換えの恩恵）／**道具屋＝消耗品（持ち物システム Phase1）＝実装済み**。残り＝書記＝伝説化承認(4-4)／ギルド＝等級・英雄譜。**持ち物システムは3段階で合意**（snapshot 4-10C 末尾）：Phase1（消耗品＋持ち物＋容量＝レベル＋道具屋）実装済み→**Phase2＝鞄（装備で容量+）**→**Phase3＝自宅保管（小容量・世代越え）**。現状まだ stub の動詞は `web/main.ts talkKeeper`。

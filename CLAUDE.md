...
> **★次スレッド最優先（2026-06-21 引き継ぎ）：** 現ライブ＝**v0.12.0**（音まわり全面＋イベント設計 P1-P4 ＋潜行途中保存＋討伐ドロップ）。**ユーザーの実機テストプレイFB待ち**＝来たら最優先で対応。体感の調整候補＝ペーシング逓増係数（`maybeDungeonEvent` の `0.12+depth*0.035`）／討伐ドロップ率（`rollKillLoot` の 15%・1/8・6%）／音量バランス。**保留タスク＝(あ)BGM 深度クロスフェード(A)＝開発が進んだら Claude から提案（①悼みのドローン未割当で確保）／(い)4-10H 第二層（山場連発防止・凪検知）／(う)M3 世界の動態。** 着手は上記ルール（設計判断は事前確認・push 前 `npm run check`）に従う。

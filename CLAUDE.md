...
1. **店・施設の他動詞の肉付け（要・小設計）**：道具屋＝**消耗品（新規アイテム型が必要）**／慰霊堂＝**鎮魂・供養（`intervene` が変質クロックをリセットする副作用＝バランス論点）**／教団＝**深蝕と引き換えの恩恵**／書記＝伝説化承認(4-4)／ギルド＝等級・英雄譜。現状これらの動詞の多くは「まだ整っていない」stub（`web/main.ts talkKeeper`）。

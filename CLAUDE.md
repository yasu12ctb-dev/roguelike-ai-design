...
- **遭-①（PR #13）／遭-②（PR #15）：** 化石遭遇がストーリーレット駆動のイベントノードに。〈調べる〉〈捜索〉で掘り下げ→effects還流（絆/深蝕/形質/年代記）＋伏線（plant→flag）で後続を解錠。
- **イベント拡充 第1弾（PR #39）：** ストーリーレットを 40→76 に増量（encounter36/dungeon25/chest15）。未活用prereq（kind/finalAct/unfinished/minExposure/stage）と伏線連鎖を全極で活用。
- **遭遇の常設動詞フル結線（PR #40・4-12B）：** 鎮魂=深蝕軽減（人間性回復）／立ち去る=未完を残す（再来の種・intervene せず）／継承=未完の目的を負う。締め文を `render.ts` の tone別プール＋rngに集約しCLI・Web共有。`intervene()` を type別分岐に。「選択の差が出ない・文が反復」を全コンテキストで解消。
...
- **遭-①** 〈調べる〉＋effects還流 **← 完了・PR #13**／**遭-②** 〈捜索〉＋伏線（plant→flag）**← 完了・PR #15**。型 Storylet/Prereq/Effect 確定（`storylets.ts`・`content/storylets.json`）。
- **イベント拡充（4-12(H)）の進め方：** (H)-1 NPC不要の2コンテキスト（遭遇・ダンジョン）を厚く **← 第1弾 完了・PR #39** → **(H)-2 アクター記述子（次の本命）**＝化石 origin を一般化し街・依頼・生者NPCを解錠（4-12(G)で★確定★） → (H)-3 街・依頼・生者を一気に拡充（第2弾）。→ 遭-③（依頼・戦闘結線）→ 遭-④（山場SetPiece）。〈戦う〉動詞で 4-11 と合流。

**【ユーザー要望・チェックポイント】イベント拡充：** 遭遇イベントは「出来るだけ多く・ランダムに近い」体験にしたい（ユーザー方針）。**節目ごとに拡充を繰り返す**（第1弾＝PR #39 済み。次は (H)-2 アクター記述子の後に第2弾＝街/依頼/生者を含めて）。詳細は `ROADMAP.md` 横断A。

...
1. **店・施設の他動詞の肉付け（要・小設計）**：慰霊堂（鎮魂・供養）／教団（深蝕と引き換えの恩恵）／**道具屋＝消耗品（持ち物システム Phase1）＝実装済み**。残り＝書記＝伝説化承認(4-4)／ギルド＝等級・英雄譜。**持ち物システムは3段階で完了**（snapshot 4-10C 末尾）：Phase1（消耗品＋持ち物＋容量＝レベル＋道具屋）・Phase2（鞄＝4つ目の装備スロット `bag`・容量+）・**Phase3＝自宅保管（1軒を `kind:"home"` 自宅化／`World.stash`＝世代越え／総容量 `STASH_CAP=12`・継承 `STASH_INHERIT=4` 枠で `fossilizeCurrent` 切詰め）実装済み**。残課題＝予備装備の保管（stash は消耗品のみ）。現状まだ stub の動詞は `web/main.ts talkKeeper`（書記/ギルドの一部）。
2. **クリア／長期目標（snapshot 4-13 候補）の設計確定**：ROADMAP が「M3前に確定」とする宿題（帰還型／モデルC型エンディング／節目連鎖）。
...
**実装の要所（街/経済/依頼/山場）：**
- 街シーン：`src/townscene.ts`（純粋・ブラウザセーフ）＋`src/web/main.ts` の `drawTown/drawInterior/townAct/interiorAct/questBoard/talkKeeper/smithBuy/healerTreat`。データ＝`content/town.json`（建物追加=配列1行／区画解禁=`guards[].locked` を外す）。
- 持ち物：`items.ts CONSUMABLES`／`progression.ts carryCapacity・STASH_CAP/STASH_INHERIT`／`types.ts InventorySlot・World.stash`＋`web/main.ts` の `storeBuy/storeSell/storeManage`（道具屋）・`homeDeposit/homeWithdraw/homeView`（自宅＝`kind:"home"`／`World.stash`＝世代越え・`fossilizeCurrent` で4枠に切詰め）・`bagBtn`（潜行中に使用＝一手消費）・`addConsumable/applyConsumable/consumeOne`。消耗品追加＝`CONSUMABLES` に1行。
...
**設計的負債：** 職業選択(流儀)は撤去済み(archetype="wanderer"固定)。`renderRumor` 出力に「深度深度N」の重複表示（既存テンプレ起因・要修正候補）。

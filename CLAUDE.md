...
- **深蝕バランス改修（2026-06-19・ユーザー承認＝「深度の刻印型」・snapshot 4-13 末尾）：** テストプレイFB「深蝕HPドレインがきつすぎ＝クリア不能」対応。**累積を毎手×歩数→降りた階数へ移行**（`variation.ts exposurePerDescent` 中0.10/深0.20×心係数。web `enterFloor` で降下時に刻む＝マップの広さに不依存。`endTurn` の毎手累積は撤去）＋**牙の上限 −2/手**（`CORRUPTION_DRAIN_CAP`・閾値`CORRUPTION_DRAIN_FROM 1.5`・`STEP 2.0`）＋**深淵帯フロアを小型固定 40×44**（`dungeon.ts genFloor` abyss・脱出スプリント化）＋**聖遺物携行の毎手深蝕 `RELIC_EXPOSURE_PER_TURN 0.12→0.03`**＋**解呪術＝その場で深蝕 −0.6 の浄化弁に転用**（`spells.ts`/`castSpell`・旧「毎手抑制」は累積撤去で無用化＝`cleanseTurns` 削除）。web 限定（CLI/demo は旧 `exposureGain` 毎手モデル据え置き）。**数値は要テストプレイ微調整。**
